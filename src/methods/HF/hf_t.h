...
=======
      /**
       * Gradient evaluation handler with Cholesky-type ERIs
       * @param Dm_skij - density matrix in the primary basis
       * @param chol    - Cholesky-type eri
       * @return Gradients for all atoms on all directions
       */
      nda::array<ComplexType, 2> eval_grad(const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                                           Cholesky_ERI auto &&chol);

      /**
       * Gradients by the 2-electron term from Cholesky-type ERIs
       * @param Dm_skij - density matrix in the primary basis
       * @param chol    - Cholesky-type eri
       * @return Gradients for all atoms on all directions
      */
      nda::array<ComplexType, 2> eval_grad_2e(const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                                              Cholesky_ERI auto &&chol);

      /**
       * Gradients by the coulomb term from Cholesky-type ERIs
       * @param Dm_skij - density matrix in the primary basis
       * @param chol    - Cholesky-type eri
       * @return Gradients for all atoms on all directions
      */
      nda::array<ComplexType, 2> eval_grad_coulomb(const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                                                   Cholesky_ERI auto &&chol);

      /**
       * Gradients by the exchange term from Cholesky-type ERIs
       * @param Dm_skij - density matrix in the primary basis
       * @param chol    - Cholesky-type eri
       * @return Gradients for all atoms on all directions
      */
      nda::array<ComplexType, 2> eval_grad_exchange(const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                                                    Cholesky_ERI auto &&chol);

      /**
       * Gradient by the 2-electron term from Cholesky-type ERIs
       * @param Dm_skij - density matrix in the primary basis
       * @param chol    - Cholesky-type eri
       * @return Gradient for one atom on one direction
      */
      ComplexType eval_grad_2e(int iatom, int direction,
                               const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                               Cholesky_ERI auto &&chol);

      /**
       * Gradient by the coulomb term from Cholesky-type ERIs
       * @param Dm_skij - density matrix in the primary basis
       * @param chol    - Cholesky-type eri
       * @return Gradient for one atom on one direction
      */
      ComplexType eval_grad_coulomb(int iatom, int direction,
                                    const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                                    Cholesky_ERI auto &&chol);

      /**
       * Gradient by the exchange term from Cholesky-type ERIs
       ...
       * @return Gradient for one atom on one direction
      */
      ComplexType eval_grad_exchange(int iatom, int direction,
                                     const nda::MemoryArrayOfRank<4> auto &Dm_skij,
                                     Cholesky_ERI auto &&chol);
